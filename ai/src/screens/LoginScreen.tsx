...
export function LoginScreen() {
  ...
  const handleGitHub = async () => {
    setError('');
    try {
      localStorage.setItem('ruwt-ai-redirect', '/dashboard');
  ...
  return (
    <ScrollView style={[styles.scroll, { backgroundColor: c.bg }]} contentContainerStyle={styles.content}>
      ...
      <View style={styles.form}>
        ...
        <Input label="Password" value={password} onChangeText={setPassword} placeholder="Your password" secureTextEntry onSubmitEditing={handleLogin} />
        <Button onPress={handleLogin} disabled={loading} fullWidth size="lg">{loading ? 'Signing in...' : 'Sign In'}</Button>
      </View>
      ...
      <Button onPress={handleGitHub} variant="outline" fullWidth size="lg">Continue with GitHub</Button>
...
const styles = StyleSheet.create({
  ...
  errorBox: { padding: spacing.md, borderRadius: radii.md },
  errorText: { fontSize: fontSizes.sm, fontFamily: fontFamily.body },
  form: { gap: spacing.md },
  divider: { borderTopWidth: 1, alignItems: 'center', marginVertical: spacing.sm },
  dividerText: { fontSize: fontSizes.sm, fontFamily: fontFamily.body, paddingHorizontal: spacing.md, marginTop: -10 },
