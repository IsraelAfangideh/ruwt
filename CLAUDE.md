...
- **Desktop app**: Tauri 2 in `/desktop` (`ai.ruwt.desktop`). CI `Release Desktop` builds a **Developer ID signed + notarized** macOS DMG + Windows NSIS, publishes GitHub Release `desktop-latest` (including `desktop-latest.json`), then retriggers `deploy-ai.yml` so files land at `/downloads/Ruwt.dmg`, `/downloads/Ruwt-Setup.exe`, and `/downloads/desktop-latest.json`. Packaged apps 0.2.0+ check that manifest in-app (SHA-256, then replace + relaunch). `tauri dev` does not self-replace. Users on 0.1.0 install 0.2.0 once by hand.
  - Signing uses the iOS App Store Connect API key (`APP_STORE_CONNECT_API_KEY_*`, team `S5G585GH4X`) to mint a Developer ID Application cert. Encrypted p12 cache: `desktop/.signing/developer-id.p12.enc`. Then `notarytool` + stapler on the DMG.
  - Finder AppleScript fails on GitHub-hosted macOS. CI builds the `.app` with Tauri, then `desktop/scripts/build-macos-dmg.py` (`dmgbuild`) writes Ruwt left / arrow / Applications right without Finder.
  - Do not codesign on a mounted DMG volume — GHA hits `internal error in Code Signing subsystem`. Do not ad-hoc re-sign a Developer ID app (that strips the ticket).
  - Fallback if Gatekeeper still blocks: `curl -fsSL https://ruwt.ai/install.sh | bash`.
  - Tauri 2 injects an inline IPC script. CSP must include `script-src 'self' 'unsafe-inline'` and `connect-src ipc: http://ipc.localhost https://ipc.localhost`. The UI reads `__TAURI_INTERNALS__.invoke` (not only `__TAURI__.core.invoke`). Missing either shows “This window is running outside the Ruwt application shell.”
