...
- **Desktop app**: Tauri 2 in `/desktop` (`ai.ruwt.desktop`). CI `Release Desktop` builds unsigned macOS DMG + Windows NSIS, publishes GitHub Release `desktop-latest` (including `desktop-latest.json`), then retriggers `deploy-ai.yml` so files land at `/downloads/Ruwt.dmg`, `/downloads/Ruwt-Setup.exe`, and `/downloads/desktop-latest.json`. Packaged apps 0.2.0+ check that manifest in-app (SHA-256, then replace + relaunch). `tauri dev` does not self-replace. Users on 0.1.0 install 0.2.0 once by hand.
  ...
  - Do not codesign on a mounted DMG volume — GHA hits `internal error in Code Signing subsystem`. Sign `Ruwt.app` before `dmgbuild`. Do not ad-hoc sign the DMG itself.
  - Sequoia/Tahoe “Apple could not verify … malware” with **Done** only is Gatekeeper on a *quarantined* unsigned app. Ad-hoc signing does not remove it. Notarization does. Until a Developer ID exists, macOS install is `curl -fsSL https://ruwt.ai/install.sh | bash` (`ai/public/install.sh`) so the copy is not quarantined.
