...
export function buildPersonalHook(
  ...
  switch (stateData.state) {
    case 'brand_new': {
      // Social proof + free credits reminder
      const socialProof = activity.recentSolves > 0
        ? `${activity.recentSolves} challenges were solved this week.`
        : `${activity.totalUsers} devs are already on the platform.`;
      if (!recommended) {
        return { text: `${name} — ${socialProof} you've got 50,000 free credits waiting.` };
      }
      return {
        text: `${name} — ${socialProof} your 50,000 free credits are still waiting. "${recommended.title}" takes about 2 minutes.`,
        challengeUrl: `https://ruwt.dev/arena/${recommended.id}`,
      };
    }

    case 'tried_stuck': {
      // Specific debugging tip
      const challenge = stateData.lastChallengeName ?? 'a challenge';
      return {
        text: `${name} — you were working on "${challenge}". most people need 2-3 attempts. try asking the AI to explain the failing test case — a different prompt sometimes cracks it.`,
        challengeUrl: recommended ? `https://ruwt.dev/arena/${recommended.id}` : undefined,
      };
    }

    case 'got_one': {
      // Leaderboard rank context + next challenge
      const rankText = stateData.leaderboardRank && stateData.leaderboardTotal > 1
        ? ` you're #${stateData.leaderboardRank} of ${stateData.leaderboardTotal} on the leaderboard.`
        : '';
      if (!recommended) {
        return { text: `${name} — first solve done.${rankText} more challenges waiting.` };
      }
      return {
        text: `${name} — first solve done.${rankText} "${recommended.title}" (${recommended.difficulty}) is a good next one.`,
        challengeUrl: `https://ruwt.dev/arena/${recommended.id}`,
      };
    }

    case 'active': {
      // Streak + rank + today's daily challenge
      const parts: string[] = [];
      if (stateData.currentStreak > 0) {
        parts.push(`${stateData.currentStreak}-day streak`);
      }
      parts.push(`${stateData.totalPassed} solves`);
      if (stateData.leaderboardRank && stateData.leaderboardTotal > 1) {
        parts.push(`#${stateData.leaderboardRank} of ${stateData.leaderboardTotal}`);
      }
      const statsLine = parts.join(', ') + '.';
      const dailyLine = activity.dailyChallenge
        ? ` today's daily: "${activity.dailyChallenge.title}" — ${activity.dailyChallenge.solveCount} solve${activity.dailyChallenge.solveCount !== 1 ? 's' : ''} so far.`
        : '';
      return { text: `${name} — ${statsLine}${dailyLine}` };
    }

    case 'dormant': {
      // Days away + what they missed
      const daysText = stateData.daysSinceLastActivity
