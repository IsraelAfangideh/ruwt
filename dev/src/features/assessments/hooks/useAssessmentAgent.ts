...
export const TOOL_SUCCESS_LABELS: Record<string, (result: any) => string> = {
  ...
  /* istanbul ignore next -- @preserve */ remove_challenges: (r) => { /* istanbul ignore next -- @preserve */ return `Removed ${r?.removed ?? 0} challenge${r?.removed === 1 ? '' : 's'}`; },
  set_weights: (r) => {
  ...
  /* istanbul ignore next -- @preserve */ set_time_limit: (r) => { /* istanbul ignore next -- @preserve */ return `Time limit set to ${r?.minutes ?? '?'} min`; },
