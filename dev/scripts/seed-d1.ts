...
Moderate AI budget ($0.015) — describe both functions clearly and concisely in a single prompt exchange.`,
    difficulty: 'easy',
...
/**
 * Warns about a description that promises the user a limit which nothing
 * enforces. Only max_cost is enforced — see functions/_shared/scoring/
 * constraints.ts. max_tokens is read by no code, so a challenge that sets it
 * and leaves max_cost NULL lies to the user.
 */
function warnUnenforcedLimits() {
  // Matches the new "AI budget" wording as well as the old "token limit" one.
  // A narrower pattern would let a rewording silence the check instead of
  // fixing it — which is exactly how these rows went unnoticed.
  const offenders = sampleChallenges.filter(
    (c) => /\b(token|ai)\s+(limit|budget)/i.test(c.description) && c.maxCost === null
  );
  if (offenders.length === 0) return;
  console.warn(
    `\nWARNING: ${offenders.length} challenge(s) promise a limit but set maxCost: null.\n` +
    'Nothing enforces the promise. Set maxCost, or reword the description.\n' +
    offenders.map((c) => `  - ${c.id}`).join('\n') + '\n'
  );
}

function main() {
  warnUnenforcedLimits();
