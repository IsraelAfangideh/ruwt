...
export const ToneSchema = z.object({
  kindness: z.number().min(0).max(1).optional(),
...
function toneBand(value?: number, lowLabel?: string, midLabel?: string, highLabel?: string): string | null {
  if (value === undefined) {
    return null;
  }
  if (value <= 0.33) {
  ...
  if (value >= 0.67) {
    return highLabel || 'high';
  }
  return midLabel || 'balanced';
}

function toneSection(tone?: { kindness?: number; formality?: number }): string {
  if (!tone || (tone.kindness === undefined && tone.formality === undefined)) {
    return '';
  }
  const kindnessLabel = toneBand(tone.kindness, 'mean/harsh', 'balanced', 'kind/empathetic');
  const formalityLabel = toneBand(tone.formality, 'casual/relaxed', 'balanced', 'formal/professional');

  return `
      TONE CONTROLS:
      - Kindness: ${tone.kindness ?? 'unset'} (${kindnessLabel})
      - Formality: ${tone.formality ?? 'unset'} (${formalityLabel})
      Apply these strongly while preserving the user's intent.
  `;
}

export function generateRespondPrompt(
  runnerName: string,
  baseSystemPrompt: string,
  userMemories: string[],
  tone?: { kindness?: number; formality?: number }
): string {
  const memoryContext = userMemories.map(m => `- ${m}`).join('\n');
  const toneContext = toneSection(tone);
