...
export default function ChatScreen({ route, navigation }: any) {
  ...
  const [tone, setTone] = useState<ToneSettings>({ kindness: 0.5, formality: 0.5 });
  ...
  const handleMakeKinder = (rewriteText: string) => {
    ...
    runnerModule.handleMessage(prompt, history, actions, tone).catch((err: any) => {
  ...
  const handleShare = async (rewriteText: string) => {
    ...
    if (result.success && result.method === 'clipboard') {
      toast.show('Sharing not supported on desktop. Copied to clipboard!', { type: 'info' });
    }
  ...
  const handleRepeatMessage = async (text: string) => {
    ...
    try {
      await runnerModule.handleMessage(text, history, actions, tone);
    } catch (error: any) {
